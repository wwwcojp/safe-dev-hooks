...
### Added
- 見つけたのに読まなかったプロジェクト設定(`.claude-hooks.json`)を通知するようになった。作業ディレクトリ側に設定ファイルが存在するのに、基準ディレクトリ(プロジェクトルート)が別の場所に解決されたために読まなかった場合、`systemMessage` で通知する(既定1時間のクールダウン、`notice_cooldown_sec` で調整。状態管理は `hooks/lib/trust.py` の新セクション `skipped_last`)。0.7.0の「無視した設定は必ず通知する」原則をこの経路にも適用したもの。
...
- 既存の `trusted_projects` の承認は原則として再承認不要(基準の変更後もキーは解決後ルートの `realpath` で一致する)。**ただし** モノレポでサブディレクトリの設定を承認していた場合は、そのエントリは使われなくなるためルートの承認が別途必要になる。
