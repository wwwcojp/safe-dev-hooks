...
**0.8.0 以降、この承認は `quality_gate` の自動検出コマンド**(`ruff` / `rustfmt` / `npx eslint`)**の実行可否にも使われます** — `trusted_projects` で承認済みのプロジェクトでのみ実行し(`false` で登録したものは「登録済み」だが非承認)、未承認のプロジェクトでは通知だけを出します。詳細: [docs/hooks/quality_gate.md](docs/hooks/quality_gate.md)。
