...
**Since 0.8.0, the same approval also gates `quality_gate`'s auto-detected lint commands** (`ruff` / `rustfmt` / `npx eslint`) — they only run for projects listed in `trusted_projects`; an unapproved project just gets a notice instead. See [docs/hooks/quality_gate.md](docs/hooks/quality_gate.md).
