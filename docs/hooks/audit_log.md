...
| マーカー | 意味 |
|---|---|
| 文字列値の末尾に付く `…[+Nc]`(`TRUNCATED_TAG_PREFIX = "…[+"`) | その値はN文字省略された。数値など非文字列の値が長すぎる場合も文字列化した上で同じ形式で切り詰める。 |
