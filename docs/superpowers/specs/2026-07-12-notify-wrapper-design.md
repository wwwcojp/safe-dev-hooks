...
4. **フォールバック**: ベル文字(`\a`)とメッセージを標準エラーへ出力する。
...
- `tests/test_notify_wrapper.py`: フォールバック経路(PATH から通知コマンドを外し、WSL 判定を無効化した状態)でベル文字とメッセージが stderr に出力されること、終了コードが0であることを確認する。
