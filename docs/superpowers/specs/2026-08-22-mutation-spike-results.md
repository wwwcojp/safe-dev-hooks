...
- **長い pragma コメントを行長制限(ruff E501 等)に収めるために文を複数行へ分割すると、mutmut の行単位マッチが外れて pragma が効かなくなる**: 分割後は変異対象のトークン(例: `ensure_ascii=False`)がコメントの付いた行と別の行になり、`# pragma: no mutate` が本来除外すべき変異を除外しなくなる。pragma を付けるときは付与直後に必ず `mutmut run` → `verify.py mutation` で該当変異が本当に消えた(スコアが上がった)ことを確認し、行分割が必要ならコメント文言を短縮して同一行に収める。
...
- **早期の型検証ループが「このキーはこの後dictとして必ず存在する」という不変条件を確立している設計では、以降の `dict.get(key, {})` の第2引数(default)は構造的に到達不能な真の等価変異になりうる**: `load_config` は `DEFAULTS.items()` を全走査してトップレベル各キーの型を検証・不一致なら既定値で上書きするループを、後続の enum/list 検証より先に実行しており、`_merge` もキーを削除しない(base の全キーを保持し override は上書きのみ)。この2条件が揃うと、後続コードの `cfg.get(<検証済みトップレベルキー>, {})` は入力に関わらず必ずキーがヒットし、`{}` フォールバックへは絶対に落ちない。pragma を付ける前に「このキーは本当に消えうるか(型検証ループの対象か・マージがキーを消しうるか)」をソースを読んで machine 的に証明してから等価と判断すること(rstripやエンコーディング名の大小のような「たまたま一致する」トラップと区別するため)。
